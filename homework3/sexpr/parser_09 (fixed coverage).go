...
func (parser *ParserImpl) sexprNT() (*SExpr, error) {
	...
	case tokenSymbol:
		// Consume NUM token
		tok, _ := parser.nextToken()
		sexpr.atom = mkTokenSymbol(tok.literal)
		return sexpr, nil
...
func (parser *ParserImpl) listNT() (*SExpr, error) {
	...
	// FIRST(<list>) = {NUM, SYMBOL, LPAR, QUOTE}
	// FOLLOW(<list>) = {RPAR}
	switch tok.typ {
	case tokenNumber, tokenSymbol, tokenLpar, tokenQuote:
		// Parse <sexpr>
		...
		// makes conCell with tail unless tail is nil -> mkNil()
		if tok.typ == tokenDot || tail != nil {
			return mkConsCell(sexpr, tail), nil

		} else {
			return mkConsCell(sexpr, mkNil()), nil
		}
...
func (parser *ParserImpl) tailNT() (*SExpr, error) {
	// peek next token to determine case to do; Checking ensured by determining how to format
	tok, _ := parser.peekToken()
	// if err != nil {
	// 	return nil, ErrParser
	// }
