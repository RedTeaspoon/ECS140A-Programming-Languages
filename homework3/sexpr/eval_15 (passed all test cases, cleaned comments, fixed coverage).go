...
func (expr *SExpr) lengthEval() (*SExpr, error) {
	...
	if err != ErrEval && eval.isConsCell() {
		// check for cons cell
		if eval.cdr != nil && eval.cdr.isNumber() {
			return nil, ErrEval
		}
...
func (expr *SExpr) zeropEval() (*SExpr, error) {
	arg1 := expr.cdr
	// check that only given one list & that current cons cell is a list
	if arg1.isNil() || arg1.cdr.cdr != nil || (arg1.cdr.isAtom() && !arg1.cdr.isNil() && arg1.cdr.atom.literal != "NIL") || arg1.car.isNil() {
		return nil, ErrEval
	}

	// println("testing: ", arg1.car.isNil())
	eval, err := (arg1.car).Eval()
	if err != ErrEval {
...
func (expr *SExpr) listpEval() (*SExpr, error) {
	...
	for i := arg1; i.car != nil; i = i.cdr {
		...
		if err == ErrEval {
			return nil, ErrEval
		} else if (i.car).isNil() || (i.car.isAtom() && !i.car.isNil() && i.car.atom.literal == "NIL") || ((i.car).isConsCell() && (eval.isNil() || eval.isConsCell())) {
			return mkSymbolTrue(), nil
		}
	}
	return mkNil(), nil
