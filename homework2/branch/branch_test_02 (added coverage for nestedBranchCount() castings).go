...
func TestComputeBranchFactors(t *testing.T) {
	...
	func double_nested_if(x int) int {
		var result int = 0
		if x < 0 {
			if x > -5 {
				result := -5
	...
	func double_nested_switch(x int) {
		switch x > 5 {
		case true:
			switch x == 6 {
			case true:
	...
	tests := []struct {
		...
		{"nested_if_no_else", 3},
		{"double_nested_if", 4},
		{"double_nested_switch", 2},
		{"nested_if_switch", 3},
		{"nested_if_typeswitch", 2},
		{"double_nested_for", 2},
		{"double_nested_range", 2},
	}
