...
func (parser *ParserImpl) termNT() (*Term, error) {
	...
	case tokenNumber:
		// Consume NUM token
		tok, _ := parser.nextToken()
		return parser.termExists(&Term{Typ: TermNumber, Literal: tok.literal, Functor: nil, Args: nil}), nil

	case tokenVariable:
		// Consume VAR token
		tok, _ := parser.nextToken()
		return parser.termExists(&Term{Typ: TermVariable, Literal: tok.literal, Functor: nil, Args: nil}), nil
...
func (parser *ParserImpl) argsNT() (*Term, error) {
	...
	if otherargs != nil {
		term.Args = append(term.Args, otherargs.Args...)
	}

	// append <otherargs> Args to <term> Args if not nil
	// <otherargs> can be single or multiple NUM | VAR | ATOM <pars>
	// <otherargs> returns \eplison
	// if otherargs == nil {
	// 	args.Args = []*Term{term}
	// 	// <otherargs> returns single NUM | VAR | ATOM <pars>
	// } else if otherargs.Literal != "" {
...
func (parser *ParserImpl) parsNT() (*Term, error) {
	...
	case tokenLpar:
		...
		// Parse <args> --> need to put args correctly in term
