...
func (parser *ParserImpl) argsNT() (*Term, error) {
	...
	// set args.Args to <term> if otherargs is nil (\eplison)
	if otherargs == nil {
		args.Args = []*Term{term}
		// otherwise put term at start of otherargs.Args array
	} else {
...
func (parser *ParserImpl) parsNT() (*Term, error) {
	...
	case tokenLpar:
		...
		// Consume RPAR; Checking ensured by otherargsNT()
		_, _ = parser.nextToken()
...
func (parser *ParserImpl) otherargsNT() (*Term, error) {
	...
	case tokenComma:
		// Consume COMMA; Checking ensured by peekToken()
		_, _ = parser.nextToken()
