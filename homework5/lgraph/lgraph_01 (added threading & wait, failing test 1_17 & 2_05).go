...
func findTarget(g1 LGraph, target node, lenLeft int, currentEdge edge, visited []rune, visitedArr *[][]rune, wg *sync.WaitGroup) {
	...
	innerWG.Add(len(edges))
	for _, edge := range edges {
		// fix shared vistedArr
		go findTarget(g1, target, lenLeft-1, edge, visited, visitedArr, &innerWG)
	}
	innerWG.Wait()
}
...
func FindSequence(g1, g2 LGraph, s, t node, k uint) ([]rune, bool) {
	...
	if s == t && g1StartExists && !g2StartExists {
		...
		wg.Add(len(g1StartEdges))
		// for every edge from start
		for _, edge := range g1StartEdges {
			// fix shared visitedArr
			go findTarget(g1, t, int(k-1), edge, visited, &visitedArr, &wg)
		}
		wg.Wait()
